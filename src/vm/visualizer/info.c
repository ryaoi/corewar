...
/*   Updated: 2019/05/21 16:58:43 by jaelee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "visualizer.h"

static void	print_champions(t_vm_state *vm, t_window *win)
{
	size_t	i;
...
static void	draw_end(t_game_data *game, t_window *win)
{
	size_t	winner;
	size_t	champ_len;
				...
				t_input_info *input_info, t_window *win)
{
	...
	box(win->info, '@', '@');
	output_info(vm, game, input_info, win);
	wattron(win->info, COLOR_PAIR(1));
	box(win->info, '@', '@');
}
